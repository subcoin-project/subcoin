...
use sp_core::H256;
use sp_runtime::generic::{DigestItem, Era};
use sp_runtime::testing::{Block, Digest, Header};
...
mod custom2 {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn optional_inherent(origin: OriginFor<T>) -> DispatchResult {
            ...
            assert!(
                !MockedSystemCallbacks::post_inherent_called(),
                "Should not already be called"
            );
            assert!(!System::inherents_applied());
    ...
    impl<T: Config> ProvideInherent for Pallet<T> {
        ...
        fn is_inherent(call: &Self::Call) -> bool {
            matches!(
                call,
                Call::<T>::inherent {} | Call::<T>::optional_inherent {}
            )
        }
    }
        ...
        for nonce in 0..=num_to_exhaust_block {
            ...
            if nonce != num_to_exhaust_block {
                ...
                assert_eq!(
                    ...
                    Weight::from_parts((encoded_len + 5) * (nonce + 1), (nonce + 1) * encoded_len)
                        + base_block_weight,
                );
                assert_eq!(
...
fn ensure_inherents_are_first_works() {
    ...
    new_test_ext(1).execute_with(|| {
        assert_ok!(
            Runtime::ensure_inherents_are_first(&Block::new(header.clone(), vec![]),),
            0
        );
        assert_ok!(
        ...
        let header = new_test_ext(10).execute_with(|| {
            ...
            for i in 0..n_in {
                let xt = if i % 2 == 0 {
                    ...
                    TestXt::new(
                        RuntimeCall::Custom2(custom2::Call::optional_inherent {}),
                        None,
                    )
                };
                Executive::apply_extrinsic(xt.clone()).unwrap().unwrap();
...
fn is_inherent_works() {
    ...
    let ext = TestXt::new(
        RuntimeCall::Custom2(custom2::Call::allowed_unsigned {}),
        None,
    );
    assert!(
        !Runtime::is_inherent(&ext),
